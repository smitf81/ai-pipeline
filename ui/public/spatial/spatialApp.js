          ...
          h('div', { className: 'button-row' },
            h('button', {
              ...
              onClick: async () => {
                ...
                try {
                  const response = await ace.askCtoDesk({ prompt, deskId: targetDeskId });
                  const answer = response?.reply || response?.report?.summary || 'No reply returned.';
                  const provenance = response?.usedDirectLlmReply
                    ? `\n\n— local model (${response?.worker?.model || 'configured'})`
                    : '\n\n— context-worker fallback';
                  setDeskChatLog((current) => [{ id: `chat-${Date.now()}-a`, role: 'ace', text: `${answer}${provenance}` }, ...current].slice(0, 10));
                } catch (error) {
