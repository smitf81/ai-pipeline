...
function renderCanonicalIntentDiagnostics(snapshot = null, meta = {}) {
  ...
  if (!resolved) {
    state.canonicalIntentSignature = 'null';
    state.canonicalIntent = null;
    setText('canonicalIntentDiagnosticsState', 'degraded');
    setText('canonicalIntentDiagnosticsSummary', 'Canonical intent diagnostics unavailable.');
    setText('canonicalIntentDiagnosticsTimestamp', 'unavailable');
    setText('canonicalIntentId', 'unknown');
    setText('canonicalIntentSourceType', 'unknown');
    setText('canonicalIntentSourceRef', 'unknown');
    setText('canonicalIntentRequestedBy', 'unknown');
    setText('canonicalIntentPriority', 'normal');
    setText('canonicalIntentTimestamp', 'unknown');
    setText('canonicalIntentStatement', 'none');
    return;
  }
...
async function refreshDashboard() {
  try {
    ...
    if (state.lastDashboardSignature) {
...
window.__ACE_APP_TEST__ = {
  ...
  refreshPlannerQaQueueDiagnostics,
