"""Build the stylized trampled-mud PBR set from the retained generated albedo.

The source is AI-generated from project grass and rock materials as style
references. This deterministic pass removes broad lighting drift, enforces an
exact quiet wrap, and derives coherent dielectric channels for compacted soil.
"""
...
def derive_albedo(source: np.ndarray) -> np.ndarray:
    ...
    broad_luminance = np.maximum(0.035, luminance(blur(periodic, 50.0)))
    target_luminance = max(0.19, float(np.median(broad_luminance)))
    correction = np.power(target_luminance / broad_luminance, 0.32)
    flattened = periodic * correction[..., None]
    # Retain dark cool loam while preserving enough value for night lighting.
    grade = np.asarray([1.02, 0.99, 0.955], dtype=np.float32)
    flattened = np.power(np.clip(flattened * grade, 0.0, 1.0), 0.985)
...
def derive_height(albedo: np.ndarray) -> np.ndarray:
    ...
    compacted_mass = blur(lightness, 1.7)
    grit_detail = lightness - blur(lightness, 4.6)
    churn_relief = blur(lightness, 4.0) - blur(lightness, 16.0)
    height = percentile_normalize(compacted_mass + grit_detail * 0.43 + churn_relief * 0.3, 1.5, 98.8)
    height = 0.1 + smoothstep(height) * 0.8
    return quiet_periodic_edges(blur(height, 0.58), EDGE_BLEND_PIXELS)


def derive_normal(height: np.ndarray, strength: float = 6.2) -> np.ndarray:
...
def derive_channels(albedo: np.ndarray, height: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    cavity = 1.0 - smoothstep(np.clip((height - 0.08) / 0.84, 0.0, 1.0))
    local_detail = np.abs(luminance(albedo) - blur(luminance(albedo), 3.0))
    ambient_occlusion = np.clip(0.96 - cavity * 0.3 - local_detail * 0.15, 0.62, 0.97)
    # Compressed damp pockets are smoother, raised clods and grit stay dry/matte.
    roughness = np.clip(0.9 - cavity * 0.14 + local_detail * 0.2, 0.72, 0.96)
