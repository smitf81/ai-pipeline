...
export function writePixelText(rects, text, x, y, options = {}) {
  const scale = options.scale ?? 2;
  const color = options.color ?? [0.9, 0.93, 0.86, 1];
  const maxWidth = options.maxWidth ?? Number.POSITIVE_INFINITY;
  let cursorX = x;
  let glyphs = 0;
  for (const char of String(text ?? '').toUpperCase()) {
    const pattern = FONT[char] ?? FONT['?'];
    const glyphW = pattern[0].length * scale;
    if (cursorX + glyphW > x + maxWidth) break;
