...
const migratedSmoke = normalizePlayerProfile(legacySmoke);
assert(migratedSmoke.progression.unlockedAbilityIds.includes(AbilityId.SMOKE_BURST), 'transition-earned smoke must persist when its canonical awakening receipt exists');
assert(migratedSmoke.progression.consumedUnlockEventIds.includes(AbilityUnlockEventId.INSTINCT_SMOKE_AWAKENED), 'the Smoke awakening receipt must survive profile normalization');
assert(migratedSmoke.progression.discoveredInstinctIds.includes(InstinctId.SMOKE_VEIL), 'legacy ability plus receipt should migrate to the stable Smoke Veil instinct id');
...
assert(capturedProfile.progression.unlockedAbilityIds.includes(AbilityId.SMOKE_BURST), 'profile capture must retain the completed Smoke awakening grant');
assert(capturedProfile.progression.consumedUnlockEventIds.includes(AbilityUnlockEventId.INSTINCT_SMOKE_AWAKENED), 'profile capture must retain the completed Smoke awakening receipt');
assert(capturedProfile.progression.discoveredInstinctIds.includes(InstinctId.SMOKE_VEIL), 'profile capture must retain the discovered Smoke Veil identity');
...
assert(canUseAbility(freshRunGame.world, freshRunGame.dragonId, AbilityId.SMOKE_BURST), 'a profile that completed First Flightless Night must retain Smoke on reload');
...
emitAccepted(instinct, InputActionId.DODGE, { dodgeMode: 'emergency', followupsEnabled: false });
step(instinct, 0.016);
...
equal(instinct.tutorial.activeCue?.supportingText, 'LOW STAMINA · RECOVER TO COUNTER', 'emergency scramble should stop advertising an unavailable pounce');
equal(instinct.tutorial.activeCue?.progress.pounceAvailable, false, 'emergency scramble should dim the unavailable contextual input');
emitAccepted(instinct, InputActionId.DODGE, { dodgeMode: 'full', followupsEnabled: true });
step(instinct, 0.016);
equal(instinct.tutorial.activeCue?.supportingText, 'SPACE AWAY · LMB TO COUNTER', 'a later funded dodge should restore the canonical pounce instruction');
equal(instinct.tutorial.activeCue?.progress.pounceAvailable, true, 'funded dodge should restore the contextual pounce stage');
emitAccepted(instinct, InputActionId.POUNCE_COUNTER);
step(instinct, 0.016);
equal(instinct.tutorial.activeCue?.phase, 'exiting', 'accepted LMB counter should complete the dodge-pounce sequence');
...
equal(pauseProjection.pauseMenu.instincts.length, 5, 'pause should project all five GDD breath instincts as marked stones');
assert(pauseProjection.pauseMenu.instincts.every((entry) => entry.discovered === false), 'a fresh profile should keep every breath-instinct stone shadowed');
assert(pauseProjection.pauseMenu.instincts.every((entry) => entry.displayName === 'UNDISCOVERED' && entry.nature == null), 'locked stones must not leak their names or nature');
