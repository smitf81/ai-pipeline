...
assert(fallingDrip.worldY < fallingDrip.groundWorldY, 'fresh napalm should visibly descend toward its ground contact point');
...
const poolLights = buildLightViews(game, 3.2).filter((light) => light.sourceKind === 'napalm_pool_light');
...
assert(napalmRecipe.light.intensity > 0.16 && napalmRecipe.light.intensity < 0.36, 'napalm pool light should be a bounded glow, not the dominant orb shape');
