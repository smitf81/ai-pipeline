...
function baseContract(settings) {
  return `
...
function managerPrompt(settings, userRequest, workspaceTree, selectedFiles) {
  return `${baseContract(settings)}
...
function builderPrompt(settings, userRequest, managerJson, selectedFiles) {
  return `${baseContract(settings)}
...
function qaPrompt(settings, userRequest, managerJson, builderJson, selectedFiles, cliResults = []) {
  return `${baseContract(settings)}
...
function repairPrompt(settings, userRequest, managerJson, builderJson, qaJson, selectedFiles, cliResults = []) {
  return `${baseContract(settings)}
...
async function runAgentSlice(settings, userRequest, selectedPaths = []) {
  ...
  const managerCall = await ollamaGenerate(settings, "manager", managerPrompt(settings, userRequest, workspaceTree, selectedFiles));
  ...
  let builderCall = await ollamaGenerate(settings, "builder", builderPrompt(settings, userRequest, managerParsed.value, selectedFiles));
  ...
  let cliResults = [];
  let qaCall = await ollamaGenerate(settings, "qa", qaPrompt(settings, userRequest, managerParsed.value, currentPlan || {}, selectedFiles, cliResults));
  ...
  while (
    ...
    const repairCall = await ollamaGenerate(
      settings,
      "repair",
      repairPrompt(settings, userRequest, managerParsed.value, currentPlan || {}, qaParsed.value || { invalidQa: qaParsed.error }, selectedFiles, cliResults)
    );
    ...
    qaCall = await ollamaGenerate(settings, "qa", qaPrompt(settings, userRequest, managerParsed.value, currentPlan, selectedFiles, cliResults));
