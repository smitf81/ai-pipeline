...
- Stay upstream. Do not create execution steps, code patches, apply actions, or deployment actions.
- Keep the packet concise and specific to the active ACE repo context.
- If planner feedback is present, address it directly instead of restating the whole brief.
- Suggested anchors must come from the provided canonical anchor set only.
- Prefer tight phrasing over exhaustive restatement.
