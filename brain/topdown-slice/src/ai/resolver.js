...
export function resolveIntentChanges(options) {
  const inspection = inspectIntentResolution(options);
  const candidates = inspection.candidates;
  candidates.rejectedEntries = Object.values(inspection.tileDiagnostics)
    .filter((diagnostic) => diagnostic.rejectionCategory)
    .map((diagnostic) => ({
      type: diagnostic.candidate?.type ?? 'paintTile',
      target: diagnostic.target,
      reason: `rejected: ${diagnostic.rejectionReason}`,
...
function evaluateTileDiagnostic({
  ...
  const projectedLocalTraversalCost = averageValues([
    projected.traversal,
    getFieldValue(fields.traversal, x + 1, y),
    getFieldValue(fields.traversal, x - 1, y),
    getFieldValue(fields.traversal, x, y + 1),
    getFieldValue(fields.traversal, x, y - 1)
  ]);
  const projectedLocalTraversal = 1 - projectedLocalTraversalCost;
  ...
  const traversalCost = Math.max(0, 1 - projectedLocalTraversal);
  ...
  const flowPenalty = (flowPressure * 0.7 + localFlowPressure * 0.3) * traversalCost;
  const traversalCostPenalty = traversalCost;
  const corridorPenalty = corridorPressure * traversalCost;
  ...
  diagnostic.scoreBreakdown = {
    ...
    projectedVisibility,
    projectedTraversal: projected.traversal,
    projectedLocalTraversal,
    traversalThreshold: TRAVERSAL_STOP_THRESHOLD,
    coverDelta: coverGain,
    visibilityDelta: visibilityGain,
    traversalCost,
  ...
  if (projectedLocalTraversal < TRAVERSAL_STOP_THRESHOLD) {
    diagnostic.rejectionCategory = 'traversal threshold';
    diagnostic.rejectionReason = 'traversal threshold';
    return diagnostic;
  }
