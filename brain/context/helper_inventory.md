...
| `ui/legacyRunnerAdapter.js` | `ui/server.js` | Bounded sync/stream fallback routes for legacy runner payloads | `ui/tests/server.test.mjs` and the complete UI gate | Remove when both fallback routes and their payload resolver are retired or migrated |
