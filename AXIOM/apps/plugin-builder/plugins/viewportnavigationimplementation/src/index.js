...
function getCameraForward(camera) {
  if (typeof camera.getWorldDirection === 'function') {
    return normaliseVector3(camera.getWorldDirection(makeVector3()));
  }
...
function panBy(dx, dy) {
  ...
  const right = getCameraRight(camera) || { x: 1, y: 0, z: 0 };
  const up = normaliseVector3(camera.up || { x: 0, y: 1, z: 0 }) || { x: 0, y: 1, z: 0 };

  addScaledVector(camera.position, right, -dx * panSpeed);
  addScaledVector(target, right, -dx * panSpeed);
  addScaledVector(camera.position, up, dy * panSpeed);
  addScaledVector(target, up, dy * panSpeed);
  if (typeof camera.lookAt === 'function') camera.lookAt(target.x, target.y, target.z);
}
...
function zoomBy(deltaY) {
  ...
  const distance = Math.max(0.001, Math.hypot(offset.x, offset.y, offset.z));
  const nextDistance = Math.max(1.2, Math.min(120, distance * (1 + Math.sign(deltaY) * 0.12)));
  const scale = nextDistance / distance;
  camera.position.x = target.x + offset.x * scale;
  camera.position.y = target.y + offset.y * scale;
  camera.position.z = target.z + offset.z * scale;
  if (typeof camera.lookAt === 'function') camera.lookAt(target.x, target.y, target.z);
}

function panHeldKeys(deltaSeconds) {
  if (!state.pointerDown || !state.camera?.position || !state.orbitTarget) return;
...
export function installViewportNavigation(ctx) {
  ...
  addListener(window, 'mousemove', event => {
    ...
    if (state.mode === 'pan') panBy(dx, dy);
    else orbitBy(dx, dy);
    event.preventDefault();
    event.stopPropagation();
  }, { capture: true });

  addListener(state.dom, 'wheel', event => {
    zoomBy(event.deltaY);
    event.preventDefault();
    event.stopPropagation();
  ...
  state.lastTick = 0;
  state.raf = requestAnimationFrame(tick);
  getLogger(ctx)?.info?.('viewportnavigationimplementation: viewport navigation installed');
...
export async function onLoad(ctx) {
  getLogger(ctx)?.info?.('viewportnavigationimplementation: loaded');
}
...
export const integrationContract = {
  ...
  "behaviour": [
    "Middle mouse drag or Alt+left drag orbits camera around orbit target.",
    "Right mouse drag or Shift+left drag pans camera and orbit target.",
